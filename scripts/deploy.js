...
async function runPreDeploymentChecks(config) {
  ...
  switch (config.target) {
    ...
    default:
      throw new Error(
        `Invalid deployment target: ${config.target}. Valid targets: local, remote, docker`
      );
  }
}
...
async function deployRemote(config) {
  ...
  // Test SSH connection
  try {
    await utils.exec(`ssh ${sshOptions} -o ConnectTimeout=10 ${sshTarget} "exit"`, {
      stdio: 'pipe',
    });
  ...
  if (await utils.commandExists('rsync')) {
    ...
    await utils.exec(`scp ${scpOptions} -r dist/* ${sshTarget}:${config.path}/`, {
      stdio: 'inherit',
    });
...
async function runHealthCheck(config) {
  ...
  switch (config.target) {
    ...
    case 'remote':
      ...
      const sshOptions = config.port !== 22 ? `-p ${config.port}` : '';
      try {
        await utils.exec(`ssh ${sshOptions} ${sshTarget} "test -f ${config.path}/package.json"`, {
          stdio: 'pipe',
        });
...
async function updateDeploymentInfo(config) {
  ...
  switch (config.target) {
    ...
    case 'remote':
      const sshTarget = `${config.user}@${config.host}`;
      const sshOptions = config.port !== 22 ? `-P ${config.port}` : '';
      await utils.exec(`scp ${sshOptions} ${deploymentInfoFile} ${sshTarget}:${config.path}/`, {
        stdio: 'pipe',
      });
      break;
